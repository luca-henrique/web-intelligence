    ...
    <section id="social-media" className="w-full py-16 md:py-24 lg:py-32 bg-gradient-to-br from-gray-50 to-white">
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
          <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors">{t("socialMedia")}</Badge>
          <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl">{t("stayConnected")}</h2>
          <p className="max-w-[800px] text-gray-600 md:text-xl/relaxed">{t("socialDescription")}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-16">
          {socialPlatforms.map((platform) => (
            ...
            >
              ...
              <span className="text-xs font-medium">{t("viewProfile")} →</span>
            </Link>
          ))}
        </div>

        <div className="mb-8">
          <h3 className="text-2xl font-bold mb-8 text-center">{t("latestPosts")}</h3>
